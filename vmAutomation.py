...
parser.add_argument('folder_name', help = "name of the folder you want to run")
...
def saveHash(ransomwareExtension, hash_value):
...
def retryHash(ransomwareExtension, hash_value):
...
while(count != config.runs):
    ...
    try:
        ...
        hash_value = extrachHash(config.hashFileName)
        ...
        runSpade()
        os.system(f'''VboxManage guestcontrol "{config.spadeVmName}" run --exe {config.getFileOperationsPath} --username "{config.spadeVmUsername}" --password "{config.spadeVmPass}" -- {config.getFileOperationsPath} {config.main_folder} {config.current_folder}''')
        closeSapde()
        ...
        saveHash(config.ransomwareExtension, hash_value)
        deleteHash(config.hashFileName)
    ...
    # if any other exception occured, close vm and resotre snapshot and restart the iteration
        ...
        os.system('Taskkill /IM Procmon64.exe /F')
        closeSapde()
        retryHash(config.ransomwareExtension, hash_value)
        deleteHash(config.hashFileName)
        continue
